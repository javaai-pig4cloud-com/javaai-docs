...
Spring AI integrates with the [Groq](https://groq.com/) by reusing the existing [OpenAI](/spring4ai/api/chat/openai-chat) client. 
For this you need to obtain a [Groq Api Key](https://console.groq.com/keys), set the base-url to https://api.groq.com/openai and select one of the 
provided [Groq models](https://console.groq.com/docs/models).

<Frame caption="Spring AI Groq Integration">
  <img src="/images/spring-ai-groq-integration.jpg" width="800" />
</Frame>

<Note>
The Groq API is not fully compatible with the OpenAI API. 
Be aware for the following [compatibility constrains](https://console.groq.com/docs/openai).
...
* **Create an API Key**:
Visit [here](https://console.groq.com/keys) to create an API Key.
...
* **Select a Groq Model**:
Use the `spring.ai.openai.chat.model=<model name>` property to select from the available [Groq Models](https://console.groq.com/docs/models).
...
<Frame caption="Spring AI Groq Functions">
...
The [OpenAiChatModel.java](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-openai/src/main/java/org/springframework/ai/openai/OpenAiChatModel.java) implements the `ChatModel` and `StreamingChatModel` and uses the low-level API to connect to the OpenAI service.
...
<AccordionGroup>
  <Accordion title="FAQ with Icon" icon="alien-8bit">
    Check out the [Accordion](mdc:content/components/accordions) docs for all the supported props.
  </Accordion>
