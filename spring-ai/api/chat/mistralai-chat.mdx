...
Check the [OpenAI API compatibility](/spring4ai/api/chat/openai-chat#openai-api-compatibility) section to learn how to use the [Spring AI OpenAI](/spring4ai/api/chat/openai-chat) integration to talk to a Mistral endpoint.
</Tip>
...
<Tip>
All properties prefixed with `spring.ai.mistralai.chat.options` can be overridden at runtime by adding request-specific [chat options](#runtime-options) to the `Prompt` call.
</Tip>
...
Mistral is OpenAI API-compatible and you can use the [Spring AI OpenAI](/spring4ai/api/chat/openai-chat) client to talk to Mistrial.
...
The [MistralAiChatModel](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-mistral-ai/src/main/java/org/springframework/ai/mistralai/MistralAiChatModel.java) implements the `ChatModel` and `StreamingChatModel` and uses the [low-level API](#low-level-api) to connect to the Mistral AI service.
