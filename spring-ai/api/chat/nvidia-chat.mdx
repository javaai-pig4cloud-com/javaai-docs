...
Spring AI integrates with the NVIDIA LLM API by reusing the existing [OpenAI](/spring4ai/api/chat/openai-chat) client. 
For this you need to set the base-url to `https://integrate.api.nvidia.com`, select one of the provided [LLM models](https://docs.api.nvidia.com/nim/reference/llm-apis#model) and get an `api-key` for it.

<Frame>
![NVIDIA LLM API Integration](spring-ai-nvidia-llm-api-1.jpg)
</Frame>
...
Check the [NvidiaWithOpenAiChatModelIT.java](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-openai/src/test/java/org/springframework/ai/openai/chat/proxy/NvidiaWithOpenAiChatModelIT.java) tests 
...
<Frame>
![NVIDIA Registration](spring-ai-nvidia-registration.jpg)
</Frame>
...
<Tip>
All properties prefixed with `spring.ai.openai.chat.options` can be overridden at runtime by adding a request specific [chat options](#runtime-options) to the `Prompt` call.
</Tip>
...
<Frame>
![NVIDIA Function Calling](spring-ai-nvidia-function-calling.jpg)
</Frame>
