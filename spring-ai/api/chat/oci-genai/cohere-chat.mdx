...
<Tip>
All properties prefixed with `spring.ai.oci.genai.chat.cohere.options` can be overridden at runtime by adding a request specific [chat options](#runtime-options) to the `Prompt` call.
</Tip>
...
Add the `spring-ai-oci-genai` dependency to your project's build file:
