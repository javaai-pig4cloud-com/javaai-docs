...
Check the [OpenAI API compatibility](/spring4ai/api/chat/openai-chat#openai-api-compatibility) section to learn how to use the [Spring AI OpenAI](/spring4ai/api/chat/openai-chat) integration to talk to an Ollama server.
</Tip>
...
* Configure and [run Ollama via Testcontainers](/spring4ai/api/testcontainers).
* Bind to an Ollama instance via [Kubernetes Service Bindings](/spring4ai/api/cloud-bindings).
...
<Tip>
All properties prefixed with `spring.ai.ollama.chat.options` can be overridden at runtime by adding request-specific [chat options](#runtime-options) to the `Prompt` call.
</Tip>
...
<Frame>
  <img src="/spring4ai/_images/multimodal.test.png" alt="Multimodal Test Image" width="200" height="200" />
</Frame>
...
In addition to the existing Spring AI model-agnostic [Structured Output Converter](/spring4ai/api/structured-output-converter), these APIs offer enhanced control and precision.
...
You can leverage existing [BeanOutputConverter](/spring4ai/api/structured-output-converter#_bean_output_converter) utilities to automatically generate the JSON Schema from your domain objects and later convert the structured response into domain-specific instances:
...
Ollama is OpenAI API-compatible and you can use the [Spring AI OpenAI](/spring4ai/api/chat/openai-chat) client to talk to Ollama and use tools.
...
<Frame>
  <img src="/spring4ai/_images/spring-ai-ollama-over-openai.jpg" alt="Ollama OpenAI API compatibility" width="800" height="600" />
</Frame>
...
The [OllamaChatModel](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-ollama/src/main/java/org/springframework/ai/ollama/OllamaChatModel.java) implements the `ChatModel` and `StreamingChatModel` and uses the [low-level API](#low-level-api) to connect to the Ollama service.
...
For more information about the `OllamaEmbeddingModel` refer to the [Ollama Embedding Model](/spring4ai/api/embeddings/ollama-embeddings) section.
</Tip>
...
The [OllamaApi](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-ollama/src/main/java/org/springframework/ai/ollama/api/OllamaApi.java) provides a lightweight Java client for the [Ollama Chat Completion API](https://github.com/ollama/ollama/blob/main/docs/api.md#generate-a-chat-completion).

The following class diagram illustrates the `OllamaApi` chat interfaces and building blocks:

<Frame>
  <img src="/spring4ai/_images/ollama-chat-completion-api.jpg" alt="OllamaApi Chat Completion API Diagram" width="800" height="600" />
</Frame>
