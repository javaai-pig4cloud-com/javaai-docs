...
Spring AI integrates with Perplexity AI by reusing the existing [OpenAI](/spring4ai/api/chat/openai-chat) client. To get started, you'll need to obtain a [Perplexity API Key](https://docs.perplexity.ai/guides/getting-started), configure the base URL, and select one of the supported [models](https://docs.perplexity.ai/guides/model-cards).

<Frame caption="Spring AI Perplexity Integration">
...
<Tip>
All properties prefixed with `spring.ai.openai.chat.options` can be overridden at runtime by adding a request specific [Runtime Options](#runtime-options) to the `Prompt` call.
</Tip>
