...
<Tip>
All properties prefixed with `spring.ai.mistralai.embedding.options` can be overridden at runtime by adding a request specific [embedding options](#embedding-options) to the `EmbeddingRequest` call.
</Tip>
...
For more information about the `MistralAiChatModel` refer to the [MistralAI Chat Client](/spring4ai/api/chat/mistralai-chat) section.
</Note>
