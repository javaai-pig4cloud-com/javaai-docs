...
<Tip>
All properties prefixed with `spring.ai.openai.embedding.options` can be overridden at runtime by adding a request specific [embedding options](#embedding-options) to the `EmbeddingRequest` call.
</Tip>
...
For more information about the `OpenAiChatModel` refer to the [OpenAI Chat Client](/spring4ai/api/chat/openai-chat) section.
</Note>
