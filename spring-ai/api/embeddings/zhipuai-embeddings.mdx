...
<Tip>
All properties prefixed with `spring.ai.zhipuai.embedding.options` can be overridden at runtime by adding a request specific [embedding options](#embedding-options) to the `EmbeddingRequest` call.
</Tip>
...
For more information about the `ZhiPuAiChatModel` refer to the [ZhiPuAI Chat Client](/spring4ai/api/chat/zhipuai-chat) section.
</Note>
