...
As an example of how counter-intuitive it can be to create an effective prompt (for example, contrasting with SQL), a [recent research paper](https://arxiv.org/abs/2205.11916) found that one of the most effective prompts you can use starts with the phrase, "Take a deep breath and work on this step by step."
...
The [Structured output conversion](/spring4ai/api/structured-output-converter#_structuredoutputconverter) employs meticulously crafted prompts, often necessitating multiple interactions with the model to achieve the desired formatting.
...
<Note>
  The GPT 3.5/4.0 dataset extends only until September 2021.
...
<AccordionGroup>
  <Accordion title="Fine Tuning" icon="wrench">
  ...
  <Accordion title="Prompt Stuffing (RAG)" icon="database">
    A more practical alternative involves embedding your data within the prompt provided to the model. Given a model's token limits, techniques are required to present relevant data within the model's context window.
    This approach is colloquially referred to as "stuffing the prompt."
    The Spring AI library helps you implement solutions based on the "stuffing the prompt" technique otherwise known as [Retrieval Augmented Generation (RAG)](#retrieval-augmented-generation).
  ...
  <Accordion title="Tool Calling" icon="toolbox">
    This technique allows registering tools (user-defined services) that connect the large language models to the APIs of external systems.
    Spring AI greatly simplifies code you need to write to support [tool calling](/spring4ai/api/tools).
  </Accordion>
</AccordionGroup>
...
<Steps>
  <Step title="Split with Semantic Boundaries">
  ...
  <Step title="Optimize for Token Limits">
...
* The [ETL Pipeline](/spring4ai/api/etl-pipeline) provides further information about orchestrating the flow of extracting data from data sources and storing it in a structured vector store, ensuring data is in the optimal format for retrieval when passing it to the AI model.
* The [ChatClient - RAG](/spring4ai/api/chatclient#_retrieval_augmented_generation) explains how to use the `QuestionAnswerAdvisor` to enable the RAG capability in your application.
...
The [Tool Calling](/spring4ai/api/tools) mechanism addresses these shortcomings.
...
Follow the [Tool Calling](/spring4ai/api/tools) documentation for further information on how to use this feature with different AI models.
...
Follow the [Evaluation Testing](/spring4ai/api/testing) documentation for further information. 